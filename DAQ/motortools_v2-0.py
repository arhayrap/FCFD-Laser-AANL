...
class Motor:
    ...
    def move_XYZ_R(self, dX=0, dY=0, dZ=0, wait_time=100, verbose=False):
        ...
        if dY:
            ...
            if verbose: print(f"Moving Y by {-1*dY} um")
        ...
        if dZ:
            ...
            if verbose: print(f"Moving Z by {-1*dZ} um")
...
def print_help(home_coordinates):
    ...
    print(
        "\nHow to use:\n"
        "  daN   Moves in a (axis) direction by N microns. \n"
        "  dy-50   Move Y by -50 um  (relative)\n"
        "  dz200   Move Z by +200 um (relative)\n"
        "  x500    Move X to 500 um  (absolute)\n"
        "  y-300   Move Y to -300 um (absolute)\n"
        "  z0      Move Z to 0 um    (absolute)\n"
        "  h<n>    Move to predefined home coordinate n (1-indexed)\n"
        f"{home_list}\n"
...
def print_position(m):
    px, py, pz = m.get_calb()
    print(f"  X: {px.Position} um  |  Y: {-1 * py.Position} um  |  Z: {-1 * pz.Position} um")
    ...
    while True:
        ...
        for cmd in tokens:
            ...
            if m_rel:
                ...
                elif axis == 'Y': m.move_XYZ_R(dY=-1*value, verbose=True)
                elif axis == 'Z': m.move_XYZ_R(dZ=-1*value, verbose=True)
                print_position(m)
                continue
